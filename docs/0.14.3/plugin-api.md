# Genisys (PocketMine-MP 分支) API 文档（插件用）— 0.14.3

> 适用版本：`VERSION = 1.1dev`（代号 `Ikaros`），`API_VERSION = 2.0.0`，`GENISYS_API_VERSION = 1.7.3`
> 目标客户端：`v0.14.x alpha`（协议 `CURRENT_PROTOCOL = 70`，网络版本 `0.14.3`）
> 运行环境：PHP >= 7.0，需要 `pthreads >= 3.1.5`、`sockets`、`yaml`、`sqlite3`、`zlib`、`curl` 扩展
> 本文方法表以 0.14.x 通用 API 为基准，对 0.14.3 适用；本版本差异见上方「版本差异速记」。
...
| `pocketmine\GENISYS_API_VERSION` | `1.7.3` | Genisys 扩展 API 版本（插件 geniapi 字段） |
| `pocketmine\MINECRAFT_VERSION` | `v0.14.x alpha` | 目标客户端版本 |
...
| `getGeniApiVersion()` | 返回 Genisys API 版本 `1.7.3` |
...
> 以下为 0.14.3 源码中实际存在的事件，按命名空间分类（`✔` 表示实现 `Cancellable` 可取消）。
