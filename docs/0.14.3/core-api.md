# Genisys (PocketMine-MP 分支) 核心改写 API 文档 — 0.14.3

> 适用版本：`VERSION = 1.1dev`（代号 `Ikaros`），`API_VERSION = 2.0.0`
> - `const GENISYS_API_VERSION = '1.7.3';`
> 协议 `CURRENT_PROTOCOL = 70`，目标客户端 `v0.14.x alpha`
...
入口 `PocketMine.php` 依次：`CompatibleClassLoader` 注册 → `Terminal::init()` → `MainLogger` → `ThreadManager::init()` → `new Server(...)`。**`Server` 构造函数即一切初始化发生之处**，以下是 0.14.3 中 `Server.php` 的关键注册调用与行号（来自源码扫描）：

| 调用 | 用处 | 锚点 |
|---|---|---|
| `Block::init` | 行 1951 | 方块注册表初始化（建立 `Block::$list`），新增方块在此后登记 |
| `Item::init` | 行 1952 | 物品注册表初始化（建立 `Item::$list`），新增物品在此后登记 |
| `LevelProviderManager::addProvider` | 行 1983, 1984, 1987 | 注册存档格式提供者（anvil/mcregion/leveldb） |
| `Generator::addGenerator` | 6 处，行 1991 … 1996 | 注册世界生成器（normal/flat/hell/void 等） |
| `Entity::registerEntity` | 44 处，行 3070 … 3114 | 实体类注册（含全部原版生物/投掷物/矿车） |
| `Tile::registerTile` | 13 处，行 3118 … 3130 | 方块实体（Tile）注册（箱子/熔炉/告示牌等） |


构造函数内被注释/可改的步骤（部分）：

    - L1959: //TextWrapper::init();
    - L1970: //set_exception_handler([$this, "exceptionHandler"]);
    - L2047: //$this->logger->info("正在生成地狱 ".$this->netherName);
...
- 协议号：`CURRENT_PROTOCOL = 70`（v0.14.x alpha 网络版本 `0.14.3`）。协议包定义见 `pocketmine/network/protocol/Info.php`。
