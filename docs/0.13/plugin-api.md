# Genisys (PocketMine-MP 分支) API 文档（插件用）— 0.13

> 适用版本：`VERSION = 1.0dev`（代号 `Amazing PHP7!`），`API_VERSION = 2.0.0`，`iTX_API_VERSION = 1.5.8`（Genisys 早期命名）
> 目标客户端：`v0.13.x alpha`（协议 `CURRENT_PROTOCOL = 38`，网络版本 `0.13.0`）
> 运行环境：PHP >= 7.0，需要 `pthreads >= 3.1.5`、`sockets`、`yaml`、`sqlite3`、`zlib`、`curl` 扩展
> 本文方法表以 0.14.x 通用 API 为基准，对 0.13 适用；本版本差异见上方「版本差异速记」。

---

> **⚠️ 本版本差异速记（相对 0.14.3 基准）**
> - 0.14.3 有但本版本缺失的事件：BlockBurnEvent、CreeperPowerEvent、EntityEatBlockEvent、EntityEatEvent、EntityEatItemEvent、EntityGenerateEvent、ItemFrameDropItemEvent、PlayerExhaustEvent、PlayerFishEvent、PlayerGlassBottleEvent、PlayerPickupExpOrbEvent、PlayerUseFishingRodEvent
...
| 常量 | 值 | 用处 |
|---|---|---|
| `pocketmine\VERSION` | `1.0dev` | 核心版本号 |
| `pocketmine\API_VERSION` | `2.0.0` | API 版本（插件 api 字段需匹配） |
| `pocketmine\iTX_API_VERSION` | `1.5.8` | Genisys 早期扩展 API 版本（旧命名，无 geniapi 字段） |
| `pocketmine\MINECRAFT_VERSION` | `v0.13.x alpha` | 目标客户端版本 |
...
api:                        # 兼容的 API 版本（必填）
- 2.0.0
itxapi:  # 0.13 旧命名                    # 兼容的 Genisys API 版本（可选，默认 ["1.0.0"]）
- 1.5.8
...
> 注意：`api` 版本比较规则——主版本号高于服务器则拒绝加载；`itxapi` 采用"主.次.补丁"严格比较。
...
> 以下为 0.13 源码中实际存在的事件，按命名空间分类（`✔` 表示实现 `Cancellable` 可取消）。
...
### 11.3 Synapse（本版本无此模块）

> 本版本（0.13）**尚未包含** Synapse 多服务器互联模块：核心中没有 `synapse/` 目录，也没有
> `isSynapseEnabled()` / `getSynapse()` 等 API。Synapse 多服互联从 **0.14.3** 起才作为核心内置模块出现。
