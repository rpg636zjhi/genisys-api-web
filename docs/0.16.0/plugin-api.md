# Genisys (PocketMine-MP 分支) API 文档（插件用）— 0.16.0

> 适用版本：`VERSION = （CI 编译时写入 git hash）`（代号 `Kyrios`），`API_VERSION = 2.1.0`，`GENISYS_API_VERSION = 1.9.3`
> 目标客户端：`v0.16.0.5 alpha`（协议 `CURRENT_PROTOCOL = 91`，网络版本 `0.16.0.5`）
> 运行环境：PHP >= 7.0，需要 `pthreads >= 3.1.5`、`sockets`、`yaml`、`sqlite3`、`zlib`、`curl` 扩展
> 本文方法表以 0.14.x 通用 API 为基准，对 0.16.0 适用；本版本差异见上方「版本差异速记」。
...
| `pocketmine\API_VERSION` | `2.1.0` | API 版本（插件 api 字段需匹配） |
| `pocketmine\GENISYS_API_VERSION` | `1.9.3` | Genisys 扩展 API 版本（插件 geniapi 字段） |
| `pocketmine\MINECRAFT_VERSION` | `v0.16.0.5 alpha` | 目标客户端版本 |
...
| `getApiVersion()` | 返回 API 版本 `2.1.0` |
| `getGeniApiVersion()` | 返回 Genisys API 版本 `1.9.3` |
...
> 以下为 0.16.0 源码中实际存在的事件，按命名空间分类（`✔` 表示实现 `Cancellable` 可取消）。
...
> 本版本（0.16.0）已把内置的 Synapse 多服务器互联模块从核心**整体移除**，核心中不再有 `synapse/` 目录。
