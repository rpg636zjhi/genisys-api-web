# Genisys (PocketMine-MP 分支) API 文档（插件用）— 0.14.0

> 适用版本：`VERSION = 1.1dev`（代号 `Ikaros`），`API_VERSION = 2.0.0`，`GENISYS_API_VERSION = 1.7.2`
> 目标客户端：`v0.14.0 alpha`（协议 `CURRENT_PROTOCOL = 45`，网络版本 `0.14.0`）
> 运行环境：PHP >= 7.0，需要 `pthreads >= 3.1.5`、`sockets`、`yaml`、`sqlite3`、`zlib`、`curl` 扩展
> 本文方法表以 0.14.x 通用 API 为基准，对 0.14.0 适用；本版本差异见上方「版本差异速记」。

---

> **⚠️ 本版本差异速记（相对 0.14.3 基准）**
> - 0.14.3 有但本版本缺失的事件：EntityEatBlockEvent、EntityEatEvent、EntityEatItemEvent、PlayerGlassBottleEvent
...
| 常量 | 值 | 用处 |
|---|---|---|
| `pocketmine\VERSION` | `1.1dev` | 核心版本号 |
| `pocketmine\API_VERSION` | `2.0.0` | API 版本（插件 api 字段需匹配） |
| `pocketmine\GENISYS_API_VERSION` | `1.7.2` | Genisys 扩展 API 版本（插件 geniapi 字段） |
| `pocketmine\MINECRAFT_VERSION` | `v0.14.0 alpha` | 目标客户端版本 |
...
| `getGeniApiVersion()` | 返回 Genisys API 版本 `1.7.2` |
...
> 以下为 0.14.0 源码中实际存在的事件，按命名空间分类（`✔` 表示实现 `Cancellable` 可取消）。
...
> 本版本（0.14.0）**尚未包含** Synapse 多服务器互联模块：核心中没有 `synapse/` 目录，也没有
